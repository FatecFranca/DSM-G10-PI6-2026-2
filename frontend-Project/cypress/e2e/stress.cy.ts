import { type Role } from '../support/factories'

const STRESS_ENABLED = ['true', '1', true].includes(Cypress.expose('stress'))
const STUDENT_COUNT = Number(Cypress.expose('stressCount')) || 20
    ...
    () => {
      ...
      Cypress._.times(STUDENT_COUNT, () => {
        cy.then(() => Cypress._.sample(institutionIds)).then((institutionId) => {
          cy.apiCreateStudent(institutionId as string).then((student) => {
            ...
            cy.apiRunAnalysis(student.id, { failOnStatusCode: false })
          })
        })
      })
      ...
      Cypress._.times(FOLLOW_UP_COUNT, () => {
        cy.then(() => Cypress._.sample(studentIds)).then((studentId) => {
          if (!studentId) return
          cy.apiCreateFollowUp(studentId as string)
        })
      })
